...
26. **官方页面日期核验**：`source_tier=官方动态` 的 `date` 必须来自具体页面正文发布日期。sitemap `lastmod`、搜索抓取时间、页面模板更新时间和地区镜像不能作为发布日；同一动态的语言/地区副本只留一个规范 URL。厂商模板把日期藏起来时（如 Qualcomm OnQ/developer blog 的 `showDate: ""`），可以用**该文章 URL 自己返回的 CMS 正文载荷**里的发布日期（如 `<article-url>.model.json` 的 `pagePublishDate`，epoch 毫秒），并与 URL 路径里的年月交叉印证；这属于一手正文数据，不是发现层时间戳。仍然核不到日期的（站点是 JS 壳、只拿得到标题，或 canonical 指向另一篇）一律丢弃。
